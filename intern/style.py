...
**드러난 것.** 인턴은 수식어를 **한 번도** 안 쓰고(대표 만자당 5.8), 접속부사가 절반이며
쉼표가 3분의 2다. 헤지만 반대 방향이다(인턴 9.5 대 대표 0).

**범인은 문체 규칙이 아니다** (2026-09-16 대조군, `scripts/modifier_ablation.py`). 같은 소재로
ⓐ현행 ⓑ`STYLE_KO`만 뺀 판 ⓒ페르소나와 분량만 준 맨몸을 돌렸다. 정도부사 밀도가
**0 · 0 · 37.8**로 나왔다. 문체 규칙을 빼도 0이고, 맨몸은 대표(11.9)의 세 배를 쓴다.
누르는 것은 금칙 목록이 아니라 **집필 프롬프트의 나머지 구조**다 - 각도·베팅·원리를 채우고
사실은 재료에 있는 것만 쓰라는 요구가 수식을 통째로 밀어낸다.
강조부사와 형용 어미는 맨몸에서도 0이라 그쪽은 모델 기본 문체에 가깝다.
