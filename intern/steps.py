...
- **판단의 근거는 표현이 아니라 규모다.** 보도자료·공지의 문구가 닮은 것은 홍보팀이 베껴 쓴 결과이지
  산업이 움직였다는 증거가 아니다. 같은 것을 말하려면 **금액·건수·지분·기간처럼 셀 수 있는 것**으로 말한다.
  「3년째 같은 문장」이 아니라 「3년째 같은 20억」이다.
...
3 **수치** - 셀 수 있는 숫자. 금액·비율·건수·기한. **같은 소재의 과거 수치를 함께 찾는다** -
  이번 라운드가 20억이면 같은 업종의 지난 라운드도 얼마였는지. 규모가 멈췄는지 움직였는지가 신호다.
  **보도자료 문구가 닮았다는 것은 수치가 아니다.**
...
**근거는 규모로 든다.** 「3년째 같은 문장」처럼 표현의 닮음을 제목에 올리지 않는다.
같은 말을 하려면 「3년째 같은 20억」처럼 셀 수 있는 것으로 쓴다.
...
- **차단(blocking)** - 이대로 내보내면 안 되는 것. 다섯뿐이다.
  ① 사실이 틀렸다 ② 논지를 받치는 핵심 물증이 없거나 출처 없는 전언이다
  ③ 각도가 약속한 것과 본문이 다루는 것이 어긋난다 ④ 논지를 뒤집는 반례를 글이 한 번도 마주하지 않는다
  ⑤ **판단의 근거로 표현·문구의 닮음을 들었다.** 보도자료 문장이 비슷하다는 것은 신호가 아니다 -
     금액·건수·지분·기간 같은 셀 수 있는 근거가 그 자리에 있어야 한다.
