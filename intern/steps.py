...
# 기준 독자 - 뉴스레터 정본(`ecri-newsletter/FORMAT-QC.md` 「독자 정의」)을 그대로 쓴다.
# 인턴이 따로 정의하면 같은 회사가 두 독자를 들게 된다(2026-09-22 대표 지시).
# **시제 판정의 관측자(TENSE_BLOCK)와 다른 축이다** - 그쪽은 레이더와 맞춰야 하는 계측기 설정이고,
# 이쪽은 무엇을 다루고 어디까지 설명할지를 정한다. 영문은 한 층이고 write_en이 따로 갖는다.
READER_KO = """[기준 독자 - 두 사람이고 순위가 아니다. 하나는 고르는 기준, 하나는 푸는 기준이다]
- **소재 층** - 케이팝을 포함한 음악업계에서 **신규 비즈니스와 확장을 고민하는 사람**.
  이 사람이 **무엇을 다룰지**를 정한다. 이 사건이 그 사람의 사업 판단에 쓰이는 자리를 본문에 만든다.
  통념을 깨는 이야기라는 것만으로는 부족하다 - 그건 설명 층이 가져가는 값이다.
- **설명 층** - 비엔터 업계인으로서 **음악 산업에 관심이 많은 사람**.
  이 사람이 **어디까지 설명할지**를 정한다. 업계에서 당연하게 쓰는 말은 처음 나올 때 한 절로 풀고
  넘어간다. 신탁단체가 무엇을 하는 곳인지, 유통사가 어디서 돈을 버는지, 카탈로그를 판다는 게
  무슨 뜻인지를 처음 듣는 말로 놓고 쓴다. **독자를 검색하러 보내지 않는다.**
- 난이도는 **업계 리포트보다 쉽고 일반 뉴스보다 깊은 자리**다."""
...
3 **수치** - 셀 수 있는 숫자. 금액·비율·건수·기한. **같은 소재의 과거 수치도 함께 찾는다** -
  지난번은 얼마였는지가 있어야 이번 것이 무엇인지 읽힌다.
...
- **사건의 경위·연혁을 요약하지 않는다.** 검색하면 나오는 배경 서사에 자리를 쓰지 않는다.
  **업계 용어는 여기서 말하는 배경이 아니다** - 용어는 위 설명 층대로 그 자리에서 한 절로 푼다.
...
2 **읽고 싶나** - 긴장이 있나. 긴장은 셋 중 하나다 - **반전 · 대비 · 미지수**.
  - 0 = 셋 중 아무것도 없다. 사실만 평평하게 적혀 있거나, **관용적 물음표뿐이다.**
    ...
    다만 **구체적 사실(수치·고유명사)이 박힌 물음은 관용구가 아니다** - 「9개월 동안 56회 공연했을까」·
    「왜 송킥을 인수했을까」는 그 글에만 붙는 물음이므로 깎지 않는다.
...
- **한국어는 합니다체로 쓴다**(「체결했습니다」). 본문과 바로 위아래 줄이 전부 합니다체라 여기만 해라체면 읽다가 걸린다(2026-09-26).
- **무리 안에 오늘 사건과 무관한 기사가 섞여 있으면 요약에서 뺀다.** 제목 낱말이 겹쳐 같이 묶인 것일 수 있다 - 요약은 사건 하나를 세우는 자리다.
